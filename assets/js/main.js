...
$(function() {
  ...
  $('.album-item').each(function() {
    const overlay = $(this).find('.album-item__overlay');
    const labelsDiv = overlay.find('.album-item__labels');
    if (!labelsDiv.length) return;
    const categories = $(this).attr('data-category').split(',').map(c => c.trim());
    labelsDiv.html(categories.map(cat =>
      `<span class="album-label">${categoryMap[cat] || cat}</span>`
    ).join(' '));
  });
});
